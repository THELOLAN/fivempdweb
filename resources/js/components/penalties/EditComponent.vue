                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-7">
                                                <select v-model="form.parent" class="form-control" :class="{ 'is-invalid' : form.errors.has('parent') }">
                                                    ...
                                                    <option :selected="penalty.id == form.parent" :value="penalty.id" v-for="penalty in penalties" v-bind:key="penalty.id">{{ penalty.label }}</option>
                                                </select>
...
export default {
    ...
    methods: {
        postForm() {
            this.form.post('/strafkatalog/'+ this.$route.params.id +'/aktualisieren')
                .then((response) => {

                        this.$noty.success("You have successfully updated the entry.", {
                            theme: 'relax',
                            timeout: 4000,
                        })
                        this.$router.push({ name: 'penalties-index' })
                })
        },
        getData() {
            axios.get('/strafkatalog/'+ this.$route.params.id +'/daten')
                .then((response) => {
                    this.form.paragraph = response.data.penalty.paragraph;
                    this.form.label = response.data.penalty.label;
                    this.form.fine = response.data.penalty.fine;
                    this.form.imprisonment = response.data.penalty.imprisonment;
                    this.form.text = response.data.penalty.additional;
                    this.form.parent = response.data.penalty.parent_id;
