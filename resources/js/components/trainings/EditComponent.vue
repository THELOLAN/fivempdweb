...
export default {
  ...
  methods: {
    getData() {
      axios.get("/ausbildungen/"+this.$route.params.slug+"/daten").then((response) => {
    ...
    postForm() {
      this.form.post("/ausbildung/"+this.$route.params.slug+"/aktualisieren").then((response) => {
        this.$noty.success("You have successfully updated the training.");
      });
    },
  },
};
